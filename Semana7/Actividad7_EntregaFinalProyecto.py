...
def mostrar_menu():
    ...
    print(f"{BLUE}3. Lector de textos {RESET}") 
        ...
        if opcion_archivo == '1':
            ...
            try:
                with open(filename, 'a', encoding='utf-8') as f:
                    f.write("\n" + content) # Añadi una nueva línea para el contenido
                print(f"Contenido añadido al archivo '{filename}' exitosamente.")
            except PermissionError:
            ...
            def heartbeat_url(url):
                try:
                    ...
                    if respuesta.status_code == 200:
                        ...
                        return False # Retorna False si no responde (pero hay status)
                except requests.exceptions.RequestException as e:
                    print(f"❤️‍💥️{full_url} no responde 💔 / Iniciando diagnostico..")
                    return False # Retorna False si hay una excepción

            is_active = heartbeat_url("google.com") # Ahora se manejará la adición de 'https://' internamente y se captura el estado
